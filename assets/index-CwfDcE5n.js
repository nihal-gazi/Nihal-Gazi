...
`,n=`
  <section id="beyond" class="viewport-section" style="display: flex; flex-direction: column; align-items: center; justify-content: center; min-height: 100vh; padding: 10vh 5vw; position: relative; overflow: hidden;">
    ...
    <h2 class="font-heavy" style="font-size: 11vw; color: #333333; white-space: nowrap; letter-spacing: -0.05em; margin: 0; line-height: 0.8; user-select: none;">
      BBBBBBEYONDDDDD
    </h2>
...
`,v=`
  <section class="dataroom-layout animate-fade-in">
    <div class="dataroom-header">
      <div class="dossier-tag font-subsub">INDEX_LOG::ACADEMIC_LEDGER</div>
      <h1 class="dataroom-title font-heavy">Scholarly Index</h1>
    </div>
    
    <div class="publications-ledger">
      ${[{id:`pub-01`,status:`published`,statusLabel:`PUBLISHED - PEER REVIEWED`,date:`MAY 29, 2026`,title:`ARGUS: The AI Eye for CERN - Solving Extreme Computational Physics`,authors:[{name:`Nihal Gazi`,isMe:!0},{name:`Aditya K. Biswas`},{name:`Aihik Basu`},{name:`Anirban Chattopadhyay`},{name:`Sayan Pratihar`}],publisher:`EPJ Web of Conferences (Volume 370, 2026)`,conference:`International Conference on Advanced Physics (IEMPHYS-26)`,scope:`High-performance physics computation & sparse vision transformer models (FLES)`,actions:[{label:`View on EPJ ↗`,url:`https://doi.org/10.1051/epjconf/202637001032`},{label:`Download PDF`,url:`https://www.epj-conferences.org/articles/epjconf/pdf/2026/26/epjconf_iemphys-26_01032.pdf`,isSecondary:!0}]},{id:`pub-02`,status:`accepted`,statusLabel:`ACCEPTED / IN PRESS`,date:`FORTHCOMING`,title:`Native Binarization: Preventing Collapse in 1-Bit Diffusion`,authors:[{name:`Nihal Gazi`,isMe:!0},{name:`Ayushman Bhattacharya`},{name:`Aditya K. Biswas`},{name:`Saubhagya Kunti`},{name:`Aihik Basu`}],publisher:`Accepted for Top 10 Best Poster Presentation`,conference:`Spectrum 2026 International Conference`,scope:`Binarized weights & stabilization algorithms for edge diffusion models`}].map(e=>{let t=e.authors.map(e=>e.isMe?`<strong>${e.name}</strong>`:e.name).join(`, `),n=`
    <span><strong>Journal / Publisher:</strong> ${e.publisher}</span>
    <span><strong>Conference:</strong> ${e.conference}</span>
    <span><strong>Scope:</strong> ${e.scope}</span>
  `,r=e.actions?`
    <div class="pub-actions">
      ${e.actions.map(e=>`
        <a href="${e.url}" target="_blank" rel="noopener noreferrer" class="btn-academic ${e.isSecondary?`secondary`:``} font-subsub">
          ${e.label}
        </a>
      `).join(``)}
    </div>
  `:``;return`
    <!-- Publication Item: ${e.id} -->
    <div class="publication-card">
      <div class="pub-status-row">
        <span class="pub-tag ${e.status}">${e.statusLabel}</span>
        <span class="pub-date font-subsub">${e.date}</span>
      </div>
      <h2 class="pub-title font-heavy">${e.title}</h2>
      <p class="pub-authors font-body">
        ${t}
      </p>
      <div class="pub-meta-info font-body">
        ${n}
      </div>
      ${r}
    </div>
  ...
  <div class="secret-container" style="background-color: #000000; min-height: 100vh; color: #ffffff; position: relative;">
    ...
    <div id="secret-blog-layer" class="secret-blog-layer">
      <div class="editorial-blog-wrapper">
        ...
        <!-- Hero Header -->
        <div class="blog-hero">
          <h1 class="blog-hero-title font-heavy">${b.hero.title}</h1>
          <p class="blog-hero-sub font-subsub">${b.hero.citation}</p>
        </div>
        ...
        <div class="blog-footer">
          <p class="conclusion-text font-body">
            Sometimes, in life, we need to slow down, and just put a little <a href="${b.footer.arxivUrl}" target="_blank" rel="noopener noreferrer" class="hidden-attention-link">attention</a>. It's a good habit. And remember, every good habit, starts at <a href="https://nihalgazi.com" class="home-router-link">home</a>.
          </p>
        </div>
